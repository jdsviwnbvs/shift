...
def train_model(model, config, X_train, y_train, X_val, y_val):
    ...
    for epoch in range(config.train_epochs):
        ...
        for batch_x, batch_y, in zip(X_train, y_train):
            ...
            loss_value = criterion(batch_x, config.freq, outputs, batch_y, torch.ones_like(batch_y))
            loss = loss_value
...
def validate(model, config, X_val, y_val, criterion):
    ...
    with torch.no_grad():
        ...
        loss = criterion(x.detach().cpu()[:, :, 0], config.frequency_map, pred[:, :, 0], true, batch_y_mark)
