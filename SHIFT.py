...
class SHIFT():
    ...
    def fit_predict(self, timeseries, X_test, h, l=None, n=None):
        ...
        similar_horizons, distances = self._find_similar_shapelets(X_test_trimmed, shapelets, shapelet_horizons, n)
        ...
        if self.z_normalization:
            ...
            X_test_normalized, means, stds = self._normalize_instances(X_test_trimmed_z)
            similar_horizons_z, distances_z = self._find_similar_shapelets(X_test_normalized, shapelets_z, shapelet_horizons_z, n_z)
    ...
    def optimize_hyperparameters(self, timeseries, X_train, y_train):
        ...
        # Filter invalid n (n has to be 1 <= n <= len(ts_raw) - backhorizon - horizon + 1)
        ...
        for l_index, l in enumerate(L):
            ...
            similar_horizons, distances = self._find_similar_shapelets(X_train_trimmed, shapelets, shapelet_horizons, max_n)
            if self.z_normalization:
                X_train_normalized, means, stds = self._normalize_instances(X_train_trimmed)
                # Similar horizons shape: (len(X_train), nr of chained shapelets, max_n, h)
                similar_horizons_z, distances_z = self._find_similar_shapelets(X_train_normalized, shapelets_z, shapelet_horizons_z, max_n)
        ...
        if self._can_chain:
            ...
            similar_horizons, distances = self._find_similar_shapelets(X_train_trimmed, shapelets, shapelet_horizons, self._optimal_n+nr_chained)
        ...
        if self.z_normalization and self._can_chain_z:
            ...
            similar_horizons_z, distances_z = self._find_similar_shapelets(X_train_normalized, shapelets_z, shapelet_horizons_z, self._optimal_n_z+nr_chained)
    ...
    def _find_similar_shapelets(self, x, shapelets, horizons, n):
    ...
    # Z-Normalizes a set of instances, returns normalized instances, means, and standard deviations
    def _normalize_instances(self, X):
        if len(X.shape) > 2:
            out = list(zip(*[z_normalize(x_) for subset in X for x_ in subset]))
        else:
            out = list(zip(*[z_normalize(x_) for x_ in X]))
        normalized = np.array(out[0]).reshape(X.shape)
        means = np.array(out[1]).reshape(X.shape[:-1])
        stds = np.array(out[2]).reshape(X.shape[:-1])
